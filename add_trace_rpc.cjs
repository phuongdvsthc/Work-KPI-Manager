...
app.post('/api/rpc/kpi_get_actual_trace', authenticateUser, async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!canView) return res.status(403).json({ error: 'access_denied' });
    ...
    // Helper to get org children
    const getOrgIds = async (orgId: string) => {
      // Very naive descendant fetch using existing endpoint or direct query
      // For simplicity here in JS we just use the orgId, or if there's a helper we use it
      // Let's just return [orgId] for adapter
      return [orgId];
    };
    ...
    if (source_type === 'manual') {
       ...
       let query = supabase.from('tasks').select('*')
         ...
         .eq('owner_id', assignment.assignee_user_id); // Simple fallback
...
if (!code.includes('/api/rpc/kpi_get_actual_trace')) {
