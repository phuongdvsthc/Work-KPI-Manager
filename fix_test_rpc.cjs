...
const newFunc = `
    ...
    await supabase.from('profiles').update({ system_role: 'admin' }).eq('id', adminUser.user.id);
    ...
    async function createTestAssignment(assigneeId, assigneeType, missingActual, isLocked) {
      const { data: assignmentId, error: asgnErr } = await sbAuth.rpc('kpi_create_assignment_from_template', {
      ...
      await supabase.from('kpi_manual_actual_entries').insert([
        { assignment_item_id: itemId1, actual_value: 100, record_date: '2026-01-10', entered_by: adminUser.user.id, status: 'approved' }
      ]);
      if (!missingActual) {
        await supabase.from('kpi_manual_actual_entries').insert([
          { assignment_item_id: itemId2, actual_value: 50, record_date: '2026-01-11', entered_by: adminUser.user.id, status: 'approved' }
        ]);
      }
      if (isLocked) {
