...
// The file should end properly. Let's find the start of executive.overview
const overviewStart = code.indexOf("'executive.overview':");
if (overviewStart !== -1) {
    let cleanCode = code.substring(0, overviewStart);
    // Remove the comma before it if it exists
...
`;
    fs.writeFileSync('src/services/ai/aiPromptRegistry.ts', cleanCode + endStr);
}
