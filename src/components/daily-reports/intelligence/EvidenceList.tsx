...
import { EvidenceRef } from '../../../types/daily-report-intelligence';

interface EvidenceListProps {
  evidence?: EvidenceRef[];
  role?: 'staff' | 'manager';
  onDrillDown?: (evidence: EvidenceRef) => void;
  maxDisplay?: number;
}
...
export const EvidenceList: React.FC<EvidenceListProps> = ({
  ...
  const validEvidence = evidence.filter(
    (ev) => ev && ev.type === 'daily_report' && ev.dailyReportId
  );
  ...
  const renderLabel = (ev: EvidenceRef) => {
    const dateFormatted = formatVNDate(ev.reportDate);
    const dateLabel = dateFormatted ? `Báo cáo ${dateFormatted}` : 'Báo cáo ngày';
    
    if (role === 'manager' && (ev.staffName || ev.userName)) {
      return `${dateLabel} – ${ev.staffName || ev.userName}`;
    }
    return dateLabel;
  };
    ...
    <div className="mt-1.5 flex flex-wrap items-center gap-1.5 text-xs">
      ...
      {overflowCount > 0 && (
        <span
          ...
          title={`Còn ${overflowCount} báo cáo khác`}
        >
          +{overflowCount} nguồn
        </span>
      )}
