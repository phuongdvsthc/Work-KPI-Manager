...
async function run() {
  ...
  const { data: user, error: updErr } = await supabaseAdmin.auth.admin.updateUserById(
    'ac7d0840-3024-402e-b5ec-08571ab238a4', // admin user id from test_users.cjs
    { password: 'password123' }
  );

  if (updErr) {
    console.log("Failed to update password:", updErr.message);
    return;
  }
  ...
  if (authErr) {
    console.log("Failed to login:", authErr.message);
    return;
  }
